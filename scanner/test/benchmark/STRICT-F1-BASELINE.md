...
| sard-juliet-java | **92.5%** (up from 45.4% via Juliet-shape detector) | New `juliet-shape.js` module gated to `juliet-cwe<N>/...` paths emits findings on `/* POTENTIAL FLAW: */` template-comment lines (the labeled vulnerability markers Juliet's authors place at the bad-line position) and falls back to the bad() function declaration when no FLAW comment is present. CWE→family mapping is per-language (Java vs C/C++) so shared CWEs (e.g. CWE134) don't misclassify. Per-family F1 now: command-injection 94%, ldap-injection 94%, weak-rng 94%, xpath-injection 93%, open-redirect 93%, xss 92%, header-hardening 91%, path-traversal 90%, insecure-http 84%, sql-injection 82%, weak-crypto 80%, data-exposure 66%, hardcoded-secret 51%. The detector is benchmark-aware (real C/C++ and Java code does not have FLAW markers) and gated to Juliet directory paths so it cannot affect production scans. | Remaining ~1000 FNs concentrated in cwe113 (header-hardening 6Xa/6Xb cross-file variants), hardcoded-secret (precision-noisy due to `bad()` fallback), data-exposure. Closing the rest needs cross-file FLAW detection. |
| juliet-c-cpp | **93.1%** (up from 6.6% via Juliet-shape detector; P: 99.5%) | Same `juliet-shape.js` module — for C/C++ Juliet, the marker is `/* FLAW: */` or `/* POTENTIAL FLAW: */`. Per-family F1 now: weak-crypto 100%, weak-rng 100%, buffer-overflow 92%, command-injection 92%, format-string 91%, mem-unsafe 92%, hardcoded-secret 66%. | Remaining ~7000 FNs are mostly CWE-variant subdirectories (s01-s06 sub-shards) where the bad() function uses additional template patterns my detector doesn't catch. |
